...
export function CapturedPieces({
  ...
  const whiteCaptures = capturedByWhite.reduce((acc, piece) => {
    if (!acc[piece.type]) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1 min-h-[40px] p-2 bg-muted/30 rounded">
            {Object.values(whiteCaptures).length === 0 ? (
              <span className="text-xs text-muted-foreground">None</span>
            ) : (
              Object.values(whiteCaptures).map((piece, index) => (
                <div
                  key={index}
        ...
        {advantage !== 0 && (
          <div className="text-center p-2 bg-muted/50 rounded">
            <span className="text-sm font-medium">
              {advantage > 0 ? "White" : "Black"} leads by {Math.abs(advantage)}{" "}
              points
            </span>
          </div>
        )}
